...
for j = 1:size_rest_data
    ...
    [n,der,center,radius,idx] = number_cluster(time',x',y',5,1,0);
    ...
    for i = 1:n
            ...
            [yy_lin,gof_lin] = fit(tr(1:end-5)',MSDr(1:end-5)',f_lin,'Startpoint',[D0]);
            [yy_exp,gof_exp] = fit(tr(1:end-3)',MSDr(1:end-3)',f_exp,'Startpoint',[(conv*radius(i)),D0/((conv*radius(i))^2)],'Robust','Bisquare');
            
            lin_or_expo = gof_lin.rsquare/gof_exp.rsquare;
            
            if lin_or_expo < 0.7
                Din = [Din;yy_exp.a^2*yy_exp.b/4];
                Rc = [Rc;sqrt(2/5)*yy_exp.a];
...
% 
% h = waitbar(0,'Fitting and extracting data...');
% chunks_number = zeros(1,size_rest_data);
% 
% for j = 1:size_rest_data
%     
%     MSD_length = length(result(isolate_idx(j)).tracking.x);
%     chunks_number(j) = floor((MSD_length-sliding_size)/sliding_step);
%     
% end
% 
% num_steps = sum(chunks_number);
% count = 0;
% 
% for j = 1:size_rest_data
%     
...
%     chunks_number(j) = floor((MSD_length-sliding_size)/sliding_step);
%     
%     D = zeros(1,chunks_number(j));
%     R_c = zeros(1,chunks_number(j));
%     t_res = zeros(1,chunks_number(j));
%     
%     for i = 1:chunks_number(j)
%         t_res(i) = result(isolate_idx(j)).tracking.time(1+(i-1)*sliding_step);
%     end
%     
%     MSD_sliding = zeros(sliding_size,chunks_number(j));
%        
%     for i = 1:chunks_number(j)
%         if exist_z == 0
...
extract = struct('Din',Din,'Dout',Dout,'Rc',Rc,'Tres',tres);
...
%         hold all
        ...
        for i = 1:n
            ...
            [yy_lin,gof_lin] = fit(tr(1:end-5)',MSDr(1:end-5)',f_lin,'Startpoint',[0.005]);
            [yy_exp,gof_exp] = fit(tr(1:end-3)',MSDr(1:end-3)',f_exp,'Startpoint',[(conv*radius(i)),0.005/((conv*radius(i))^2)],'Robust','Bisquare');
